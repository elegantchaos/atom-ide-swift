...
export default class Swift extends Runner {
    ...
    build(stdout, stderr, useBuilder) {
        if (useBuilder) {
            return this.start(['run', 'builder', 'build'], stdout, stderr);
        } else {
            return this.start(['build'], stdout, stderr);
        }
    }

    test(stdout, stderr) {
        return this.start(['test'], stdout, stderr);
    }
