...
export default class IdeSwiftView {
    ...
    constructor(controller) {
        ...
        if (this.controller.hasExecutable()) {
            this.addOutput(`(target currently set to: \`${this.controller.executable()}\`; use \`e=nameOfExecutable\` to change it)`)
        }
    ...
    buildApp(runWhenBuilt) {
        ...
        this.swiftBuild = spawn(this.controller.swiftPath(), ['build', '--package-path', controller.workspacePath()]);
    ...
    runDebugger() {
        this.lldb = spawn(this.controller.lldbPath(), [controller.workspacePath()+"/.build/debug/"+this.controller.executable()]);
