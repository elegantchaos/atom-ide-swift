...
export default class IdeSwiftView {
    ...
    showWelcome() {
        ...
        const target = controller.executable();
        if (target == "") {
            this.addOutput(`No target set.`);
        } else if (controller.executableDidDefault()) {
            this.addOutput(`Target defaulted to: \`${target}\`.`);
        } else {
            this.addOutput(`Target: \`${target}\`.`)
        }

        this.addOutput(`(use \`t=nameOfTarget\` to change the target)`);
    }
    ...
    buildApp(runWhenBuilt) {
        ...
        const controller = this.controller;
        if(!controller.hasExecutable()) {
            this.addOutput("Please set the executable to build, using `e=nameOfExecutable`.");
            return;
        }

        controller.clearDiagnostics();

        this.swiftBuild = spawn(controller.swiftPath(), ['build', '--package-path', controller.projectRoot()]);
    ...
    runDebugger() {
        const controller = this.controller;
        this.lldb = spawn(controller.lldbPath(), [controller.projectRoot()+"/.build/debug/"+controller.executable()]);
    ...
    parseCommand(command) {
        ...
        if (/e=(.*)/.test(command)) {
            ...
            this.controller.setExecutable(match[1]);
