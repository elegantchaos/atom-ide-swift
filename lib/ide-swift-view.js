...
export default class IdeSwiftView {
    ...
    runDebugger() {
        const controller = this.controller;
        this.lldb = spawn(controller.lldbPath(), [controller.projectRoot()+"/.build/debug/"+controller.target()]);
    ...
    parseCommand(command) {
        ...
        if (/e=(.*)/.test(command)) {
            ...
            this.controller.setTarget(match[1]);
            this.addOutput(`Executable path set to \`${this.controller.target()}\`.`);
        } else {
