...
export default class IdeSwiftView {
    ...
    buildApp(runWhenBuilt) {
        ...
        this._builder = controller.swift().build(stdout, stderr, controller.shouldUseBuilder()).then( code => {
    ...
    processBuilderOutput(output) {
        ...
        while (match = pattern.exec(output)) {
            ...
            const error = { file: match[1], line: match[2], column: 0, description: `test ${test} failed: ${reason}` }
            this.controller.outputError(error);
            matched = true;
        }
