...
export default class IdeSwiftController {
        ...
        spm.getDescription().then( (description) => {
            ...
            if (name) {
                ...
                if (!this.hasExecutable()) {
                    this.setExecutable(this.defaultTarget().name);
                    this._didDefault = this.hasExecutable();
                }
    ...
    setupToolBar(toolBar) {
        ...
        buttons.test = toolBar.addButton({ icon: 'trash', iconset: 'fa', tooltip: "blah", callback: (v) => {
            const options = {
                message:"Error",
                detailedMessage:"This is a error dialog~",
                buttons: ["Yes", "No"]
            };
            atom.confirm(options, response => {
                    if (response === 0) {
    ...
    defaultTarget() {
        ...
        if (targets.executable.length > 0) {
            return targets.executable[0];
        } else if (targets.library.length > 0) {
            return targets.library[0];
        } else if (targets.test.length > 0) {
            return targets.test[0];
        } else {
            return "";
        }
    }
