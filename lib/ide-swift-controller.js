...
export default class IdeSwiftController {
    ...
    activate(serializedState) {
        ...
        if (serializedState.targetSpec) {
            this._target = serializedState.targetSpec;
        }
            ...
            products.forEach( product => {
                ...
                if ((type == "executable") && productTargets) {
                    productTargets.forEach( target => {
                        targets[target].executable = product.name;
                        console.log(`Executable for ${target.name} is ${product.name}.`)
                    });
                }
            });
    ...
    setTarget(target) {
        const changed = this._target.name != target.name;
        this._target = target;
        return changed;
    }
    ...
    targetExecutable() {
        let executable = this._target.executable;
        return executable ? executable : this._target.name;
    }
