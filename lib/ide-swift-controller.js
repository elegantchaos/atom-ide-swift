...
export default class IdeSwiftController {
    ...
    interrogatePackage(showView) {
        ...
        spm.getPackage().then( package => {
            if (package.error || package.exception) {
                console.log("Failed to get package information:")
                if (package.error) { console .log(package.error) }
                if (package.exception) { console.log(package.exception) }
            } else {
                this._package = package;
                this.interrogateDescription(spm);
            }
        });
    }

    interrogateDescription(spm) {
        spm.getDescription().then( description => {
            if (description.error || description.exception) {
                console.log("Failed to get package description.")
                if (description.error) { console.log(description.error) }
                if (description.exception) { console.log(description.exception) }
            } else {
                const name = this._packageName = description.name;
                let targets = this._targets = { test: [], library:[], executable:[], byName:{} };
                if (name) {
